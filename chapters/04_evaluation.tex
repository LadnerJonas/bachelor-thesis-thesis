...
%   \resizebox{\textwidth}{!}{
%     \begin{tabular}{|p{3cm}|>{\raggedleft\arraybackslash}p{3.5cm}|>{\raggedleft\arraybackslash}p{3cm}|>{\raggedleft\arraybackslash}p{1.5cm}|>{\raggedleft\arraybackslash}p{3cm}|>{\raggedleft\arraybackslash}p{1.5cm}|>{\raggedleft\arraybackslash}p{1.5cm}|>{\raggedleft\arraybackslash}p{1.5cm}|}
%       \hline
%       \textbf{Metric}                                       & \textbf{CmpExclPartition\-Ranges} & \textbf{CmpProcessing\-Units} & \textbf{Hybrid} & \textbf{LocalPages\-AndMerge} & \textbf{OnDemand} & \textbf{Radix} & \textbf{Smb*} \\
%       \hline
%       \textbf{Total heap allocation [GiB]}                  & 1,572                             & 2,270                         & 1,568           & 2,508                         & 1,558             & 2,564          & 1,563         \\
%       \hline
%       \textbf{Heap allocations without slotted pages [GiB]} & 0.117                             & 0.814                         & 0.113           & 1.053                         & 0.103             & 1.109          & 0.108         \\
%       \hline
%       \textbf{Overhead (\%)}                                & 8.02\%                            & 56.0\%                        & 7.75\%          & 72.34\%                       & 7.06\%            & 76.20\%        & 7.41\%        \\
%       \hline
%     \end{tabular}
%   }
%   \caption{Peak Memory Usage for Tuple 16B, 32 Partitions, and 67.2M Tuples}
%   \label{tab-peak-memory-16-32-67Mio}
% \end{table}
% \begin{figure}[h]
%   \centering
%   \begin{tikzpicture}
%     \begin{axis}[
%         ybar stacked,
%         symbolic x coords={CmpExclPartitionRanges, CmpProcessingUnits, Hybrid, LocalPagesAndMerge, OnDemand, Radix, Smb*},
%         xtick=data,
...
%       ]
%       % Blue part (Remaining heap allocation)
%       \addplot[color=blue, fill=blue!60] coordinates {
%           (CmpExclPartitionRanges,1455)
%           (CmpProcessingUnits,1456)
%           (Hybrid,1455)
%           (LocalPagesAndMerge,1455)
%           (OnDemand,1455)
%           (Radix,1455)
%           (Smb*,1455)
%         };
%       % Red part (Heap allocation without slotted pages)
%       \addplot[color=red, fill=red!60] coordinates {
%           (CmpExclPartitionRanges,117)
%           (CmpProcessingUnits,814)
%           (Hybrid,113)
%           (LocalPagesAndMerge,1053)
%           (OnDemand,103)
%           (Radix,1109)
%           (Smb*,108)
%         };
%       % \addplot[black, thick, dashed] {1455};
%       % Baseline at 1,456 MiB
%       % \addplot[black, thick, dashed] coordinates {(CmpExclPartitionRanges,1456) (Smb*,1456)};

%       % \addplot[black, thick, dashed] {1456};
%       % \addplot[black, thick, dashed] coordinates {
%       %     (CmpExclPartitionRanges,1456)
%       %     (CmpProcessingUnits,1456)
%       %     (Hybrid,1456)
%       %     (LocalPagesAndMerge,1456)
%       %     (OnDemand,1456)
%       %     (Radix,1456)
%       %     (Smb*,1456)
%       %   };

%       \legend{Heap Allocation without Slotted Pages, Total Heap Allocation, Required Slotted Pages (1,456 MiB)}
%     \end{axis}
%   \end{tikzpicture}
%   \caption{Peak Memory Usage for Tuple 16B, 32 Partitions, and 67.2M Tuples}
%   \label{fig:peak-memory-16-32-67Mio}
% \end{figure}
We used the heap profiler Massif within Valgrind 3.24 to messure the heap memory consumption of our implementations.
In our memory benchmark, we partition 67.2 Mio.
tuples of 16 bytes ($\approx$ 1 GiB) into 32 partitions.
As the tuple keys are distributed uniformly, the tuples are stored on ten 5 MiB slotted pages per partition.
...
These 10 slotted pages per partition sum up to around 1.46 GiB of heap memory, which each implementation must use and acts as an lower-bound in the plot.
To better visiualize the peak memory consumption of each implementation, we subtract the needed memory for the minimum number of slotted pages, and write the addititonally used heap memory above the bar of each implementation.

All implementations except LocalPagesAndMerge and Radix, only rely on a few, small heap allocations (vectors, SMBs).
The OnDemand does use the least amount of heap, as it does not use any \ac{SMB}.
In the group of implementations, that only use few heap allocations, the two \ac{CMP} approaches use the most heap.
They both use \acp{SMB} and their producer-consumer approach uses vectors.

The Radix implementation needs to fully materilize the 1 GiB of tuple data, before it can write out tuples to the slotted pages.
This materialization allocation directly scales with the total size of the incoming tuples.
The Hybrid approach avoids this by only materializing small chunks and directly assigning them to threads.
...
Instead, it is caused by the last, not fully used thread-local page of each partition.
These are the pages that have to be merged, so that we pass the minimal number of slotted pages to the next operator.
As we can have at most one not fully used thread-local slotted page per partition and thread, the amount of slotted pages can be upperbounded by a constant:
\begin{equation}
  c_\textrm{LPAM} \leq (\textrm{Partition count}) * (\textrm{Thread count}) * (\textrm{Size of a slotted page})
\end{equation}
This constant $c_\textrm{LPAM}$ has to be considered when using the LocalPagesAndMerge approach, as it can be serveral GiB like seen in the figure \ref{plot-heap-16B-P32-Th40}.
